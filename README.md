...
수도권 신입 개발자가 놓치기 쉬운 채용·해커톤·교육·대외활동을 먼저 찾아 Discord로 알려주고, AI 시대의 개발 변화를 오전과 저녁에 읽기 좋게 묶어주는 자동화 레이더입니다.
...
- **개발 트렌드는 하루 두 번 묶어서 받습니다.** 공식 뉴스, 국내 기술 블로그, GeekNews 인기글, 선별된 YouTube 영상을 오전 9시와 오후 6시(KST)에 Brief로 묶습니다.
...
기회 알림의 집중도를 해치지 않도록 개발 콘텐츠는 실시간으로 쏟아내지 않고 오전 9시와 오후 6시(KST)에 묶어서 전달합니다. 최근 콘텐츠를 점수화한 뒤 최신순이 아닌 **읽을 가치가 높은 순서**로 최대 6개를 고릅니다.
...
설정은 [`config/profile.json`](config/profile.json)과 [`config/sources.json`](config/sources.json)에서 변경할 수 있습니다. 내장 출처는 코드가 기본값을 제공하고, `sources`와 `briefSources`의 동일한 ID 항목이 활성화 여부·우선순위·URL 같은 운영 값을 덮어씁니다.
