...
function normalizeOpportunity(raw, now = new Date()) {
  ...
  if (!Object.values(OPPORTUNITY_TYPES).includes(raw.type)) throw new TypeError(`unsupported opportunity type: ${raw.type}`);
  ...
  const contentBasis = JSON.stringify({
    title,
    organization,
    status: raw.status || OPPORTUNITY_STATUS.OPEN,
