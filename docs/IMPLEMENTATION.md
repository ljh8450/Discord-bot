...
- 링커리어·캠퍼스픽 최근 7일 페이지네이션과 실행 내 상세 URL 캐시
- 이벤터스 공개 검색 API의 카테고리·키워드 페이지네이션
- GIWA GASOK 같은 공식 개발 프로그램을 위한 범용 official-opportunity 어댑터
- 출처별 목록·상세·승인·탈락 통계와 npm run diagnose 제목 진단
- 데이콘·데이커, GDG, 프로그래머스 공식·개별 출처 보강
- 원문 URL과 제목·마감일 기반 플랫폼 간 중복 제거
- OpenAI·GitHub·Google Developers 공식 RSS와 Anthropic 뉴스룸 기반 일일 브리프
- GeekNews 추천 점수 기반 인기글과 NAVER D2·카카오·토스·당근·우아한형제들·LINE/LY 기술 블로그 브리프
- RSS·Atom 통합 수집과 관련성·실용성·신선도·권위·커뮤니티 점수 기반 선별
- 실밸개발자·AI Frontier Korea·Tech Bridge·미드나잇 로그·조코딩·AgentOS·코딩하는 기술사·코딩애플·Chase AI YouTube 일반 영상
- EO Korea·안될공학의 AI·개발 주제 영상과 90일 휴면·자동 복구 처리
...
1. npm test
2. 저장소 루트의 .env에 카테고리별 DISCORD_*_WEBHOOK_URL을 설정한다.
3. npm run radar
...
- NAVER Z Careers
- 카카오 테크 채용 공개 API
- 링커리어: 공모전·교육·대외활동 핵심 소스
- 캠퍼스픽, 티켓타코: 보조 소스
- 이벤터스: IT·과학기술·커리어 카테고리 및 해커톤·AI Agent·빌더·MVP·개발자 키워드 검색
- 공식 개발 프로그램: 설정된 공식 페이지에서 원문 링크를 우선하는 범용 수집
- 데이콘·데이커, GDG Korea, 프로그래머스 데브코스: 개별·공식 보강 소스
- OpenAI News, Anthropic Newsroom, GitHub Blog, Google Developers Blog: 공식 AI·개발 업데이트
- GeekNews 인기글: 홈페이지 상위 20위 중 추천 5점 이상
- NAVER D2, Kakao Tech, 토스, 당근, 우아한형제들, LINE·LY 기술 블로그: 국내 현업 기술 사례
- YouTube: 핵심 9개 채널과 주제 필터를 적용한 선택 2개 채널

## 누락 진단

`npm run diagnose -- "제목"`은 현재 전체 출처를 다시 수집한 뒤 제목의 구두점과
공백을 무시하고 후보를 찾는다. 발견된 항목은 정규화·최소 검증·프로필 필터의 현재
판정을 보여준다. 발견되지 않으면 상태 파일의 과거 항목과 함께 출처별 페이지 수,
목록 수, 상세 요청 수, 매핑·탈락·중복 수 및 종료 사유를 출력한다.

일반 radar와 dry-run 출력에도 `sourceStats`와 `report.bySource`가 포함된다.
